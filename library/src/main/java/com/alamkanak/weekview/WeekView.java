...
public class WeekView extends View {
    ...
    private EventFinishDrawListener mEventFinishDrawListener;
...
//            }
    ...
    private void init() {
        ...
        //ljt modify
        mNowTimeLinePaint= new Paint();
        mNowTimeLinePaint.setStrokeWidth((float)1.0);
        mNowTimeLinePaint.setColor(Color.BLACK);
            ...
            for (int i = 0; i < mEventRects.size(); i++) {
                if (isSameDay(mEventRects.get(i).event.getStartTime(), date) && !mEventRects.get(i).event.isAllDay()){
                    ...
                    if (left < right &&
                        ...
                        //ljt modify
                        canvas.drawLine(0,moveLineTop,getWidth(),moveLineTop,mNowTimeLinePaint);
                    }
                    else
...
//                sortAndCacheEvents(nextPeriodEvents);
    ...
    public void setEventFinishDrawListener (EventFinishDrawListener eventFinishDrawListener) {
        this.mEventFinishDrawListener = eventFinishDrawListener;
    }
...
//                        SimpleDateFormat sdf = DateFormat.is24HourFormat(getContext()) ? new SimpleDateFormat("HH:mm", Locale.getDefault()) : new SimpleDateFormat("hh a", Locale.getDefault());
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if(event.getAction()==MotionEvent.ACTION_MOVE) {
            nowTime = getTimeFromPoint(event.getX(), event.getY());
            float mStartY = mHeaderHeight + mHeaderRowPadding * 2 + mTimeTextHeight/2 + mHeaderMarginBottom + mCurrentOrigin.y;
            float mBeforeNow = (nowTime.get(Calendar.HOUR_OF_DAY) + nowTime.get(Calendar.MINUTE)/60.0f) * mHourHeight;
            moveLineTop=mStartY + mBeforeNow;
        }
        //ljt modify
        if(isEventLongPress) {
            if (mEventLongPressListener != null && mEventRects != null) {
                List<EventRect> reversedEventRects = mEventRects;
                Collections.reverse(reversedEventRects);
                for (EventRect e : reversedEventRects) {
                    switch (event.getAction()) {
                        case MotionEvent.ACTION_MOVE:
                            Calendar changeTime = getTimeFromPoint(event.getX(), event.getY());
                            if(changeTime!=null) {
                                mEventLongPressListener.onEventLongPress(e.originalEvent, e.rectF,changeTime,isStartOrEndTime);
                            }
                            break;
            ...
            if (mEmptyViewLongPressListener != null && event.getX() > mHeaderColumnWidth && event.getY() > (mHeaderHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom)) {
                switch (event.getAction()) {
                    ...
                    case MotionEvent.ACTION_UP:
                        ...
                        startOrEnd=false;
                        mEventFinishDrawListener.onEvenFinishDraw();
                        break;
                }
            }
        }
    ...
    public interface EventFinishDrawListener {
        void onEvenFinishDraw();
    }
}
